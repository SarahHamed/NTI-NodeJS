...
const getData = async(callback) =>{
    ...
    let y = await(x.json())
   //  console.log(y['Countries'])   
    // return y
    callback(y.Countries, false)
    }
    catch(e){
        callback(false, e)
    }
...
// let myData = getData()
// console.log(myData)
elements=["ID","Country","CountryCode","Slug","NewConfirmed","TotalConfirmed","NewDeaths","TotalDeaths","NewRecovered","TotalRecovered","Date"]
getData( (data, error)=> {
    if(data){//console.log(data)

    data.forEach((elem,i)=>{
       // console.log(elem.Country)
        document.querySelector('.row').innerHTML=document.querySelector('.row').innerHTML+`<div class="col-md-4"><div class="m-2  bg-dark text-secondary p-4 text-center"></div></div>`
        elements.forEach(el=>{
            document.querySelectorAll('.m-2')[i].innerHTML  = document.querySelectorAll('.m-2')[i].innerHTML+`<p>`+el+`: `+elem[el]+`</p>`;
           console.log(elem[el])
        })
       // document.querySelector('.row').innerHTML=document.querySelector('.row').innerHTML+`</div>`
       // console.log(document.querySelector('.row').innerHTML)
